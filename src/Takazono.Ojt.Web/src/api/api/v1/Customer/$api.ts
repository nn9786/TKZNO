...
const api = <T>({ baseURL, fetch }: AspidaClient<T>) => {
  ...
  const PATH0 = '/api/v1/Customer/Create'
  const PATH1 = '/api/v1/Customer/Delete'
  const PATH2 = '/api/v1/Customer/Get'
  const PATH3 = '/api/v1/Customer/Search'
  const PATH4 = '/api/v1/Customer/Update'
  const PATH5 = '/api/v1/Customer/UpdateDisplayOrder'
  const GET = 'GET'
  const POST = 'POST'
  const PUT = 'PUT'
  const DELETE = 'DELETE'
  ...
  return {
    ...
    UpdateDisplayOrder: {
      ...
      $put: (option: { body: Methods_o4f9yd['put']['reqBody']; config?: T | undefined }) =>
        fetch<void, BasicHeaders, Methods_o4f9yd['put']['status']>(prefix, PATH5, PUT, option)
          .send()
          .then((r) => r.body),
      $path: () => `${prefix}${PATH5}`,
    },
