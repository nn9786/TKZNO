  ...
  return useMemo(
    () =>
      z.object({
        ...
        postalCode: z.string().max(8, getLabel('V0002', { max: '8' }) /* {max}文字以内で入力してください。 */).optional().or(z.literal('')),
        address: z.string().max(200, getLabel('V0002', { max: '200' }) /* {max}文字以内で入力してください。 */).optional().or(z.literal('')),
        phoneNumber: z.string().max(20, getLabel('V0002', { max: '20' }) /* {max}文字以内で入力してください。 */).optional().or(z.literal('')),
        useFlag: z.boolean(),
      }),
    [getLabel],
  )
