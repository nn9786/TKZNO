...
import { searchCustomers, updateCustomerDisplayOrder } from '@/services/customerApi'
...
export const CustomerDisplayOrderDrawer = ({ open, onClose, onSaved }: Props) => {
  ...
  const handleSave = async () => {
    await api(() => updateCustomerDisplayOrder({ orderedSids: items.map((item) => item.sid!) }), {
      ...
      <Box sx={styles.drawerBody}>
        ...
        <Box sx={styles.listArea}>
          <DndTableHead>
            ...
            <Box sx={{ width: 140 }}>{getLabel('T0084') /* 得意先コード */}</Box>
            <Box sx={{ flexGrow: 1 }}>{getLabel('T0085') /* 得意先名称 */}</Box>
          </DndTableHead>
          <DndTableBody droppableId="customer-display-order" onDragEnd={handleDragEnd}>
