...
import { searchSuppliers, updateSupplierDisplayOrder } from '@/services/supplierApi'
...
export const SupplierDisplayOrderDrawer = ({ open, onClose, onSaved }: Props) => {
  ...
  const handleSave = async () => {
    await api(() => updateSupplierDisplayOrder({ orderedSids: items.map((item) => item.sid!) }), {
      ...
      <Box sx={styles.drawerBody}>
        ...
        <Box sx={styles.listArea}>
          <DndTableHead>
            ...
            <Box sx={{ width: 140 }}>{getLabel('T0075') /* 取引先コード */}</Box>
            <Box sx={{ flexGrow: 1 }}>{getLabel('T0076') /* 取引先名称 */}</Box>
          </DndTableHead>
          <DndTableBody droppableId="supplier-display-order" onDragEnd={handleDragEnd}>
