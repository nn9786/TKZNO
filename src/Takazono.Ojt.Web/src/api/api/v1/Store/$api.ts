...
import type { Methods as Methods_gll2wl } from './GetAll'
...
const api = <T>({ baseURL, fetch }: AspidaClient<T>) => {
  ...
  const PATH2 = '/api/v1/Store/DownloadCsv'
  const PATH3 = '/api/v1/Store/Get'
  const PATH4 = '/api/v1/Store/GetAll'
  const PATH5 = '/api/v1/Store/Search'
  const PATH6 = '/api/v1/Store/Update'
  const PATH7 = '/api/v1/Store/UpdateDisplayOrder'
  const GET = 'GET'
  const POST = 'POST'
  const PUT = 'PUT'
  const DELETE = 'DELETE'
  ...
  return {
    ...
    GetAll: {
       ...
       */
      get: (option?: { config?: T | undefined } | undefined) =>
        fetch<Methods_gll2wl['get']['resBody'], BasicHeaders, Methods_gll2wl['get']['status']>(
          prefix,
          PATH4,
          GET,
          option
    ...
    UpdateDisplayOrder: {
      put: (option: { body: Methods_o4f9yd['put']['reqBody']; config?: T | undefined }) =>
        fetch<void, BasicHeaders, Methods_o4f9yd['put']['status']>(prefix, PATH7, PUT, option).send(),
      $put: (option: { body: Methods_o4f9yd['put']['reqBody']; config?: T | undefined }) =>
        fetch<void, BasicHeaders, Methods_o4f9yd['put']['status']>(prefix, PATH7, PUT, option)
          .send()
          .then((r) => r.body),
      $path: () => `${prefix}${PATH7}`,
    },
